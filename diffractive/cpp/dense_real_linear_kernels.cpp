...
void trilinear_real_projection_forward_cpu_kernel(
	const torch::TensorAccessor<scalar_t, 3> image_in,
	const int size_in,
	const scalar_t *rot_matrix,
	const int batch_size,
	torch::TensorAccessor<scalar_t, 3> image_out,
	const int size_out)
{
...
template <typename scalar_t>
void trilinear_real_projection_backward_cpu_kernel(
	const torch::TensorAccessor<scalar_t, 3> image_in,
	const int size_in,
	const scalar_t *rot_matrix,
	const int batch_size,
	const int size_out,
	const torch::TensorAccessor<scalar_t, 3> dcost_doutput,
	      torch::TensorAccessor<scalar_t, 3> dcost_dimage_in,
...
void trilinear_real_projection_forward_cpu(
	...
	torch::Tensor image_out)
{
	...
	AT_DISPATCH_FLOATING_TYPES(
		...
		"trilinear_real_projection_forward_cpu_kernel",
		([&] {
				trilinear_real_projection_forward_cpu_kernel<scalar_t>(
					image_in.accessor<scalar_t, 3>(),
					size_in,
					rot_matrix.data_ptr<scalar_t>(),
					rot_matrix.size(0),
					image_out.accessor<scalar_t, 3>(),
					size_out
			);
		})
	);
...
void trilinear_real_projection_backward_cpu(
    const torch::Tensor image_in,
	const torch::Tensor rot_matrix,
...
)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        rot_matrix.scalar_type(),
        "trilinear_real_projection_backward_cpu_kernel",
        ([&] {
            trilinear_real_projection_backward_cpu_kernel<scalar_t>(
				image_in.accessor<scalar_t, 3>(),
				size_in,
				rot_matrix.data_ptr<scalar_t>(),
				rot_matrix.size(0),
				size_out,
                dcost_doutput.accessor<scalar_t, 3>(),
                dcost_dimage_in.accessor<scalar_t, 3>(),
