	...
	torch::Tensor rot_matrix)
{
	...
	{
		trilinear_real_projection_forward_cpu(image, rot_matrix, output);
	}
	else
	{
		trilinear_real_projection_forward_cuda(image, rot_matrix, output);
	}
	...
	torch::Tensor dcost_doutput)
{
	...
	if (image.device().type() == torch::kCPU)
	{
		trilinear_real_projection_backward_cpu(
			image,
			rot_matrix,
			dcost_doutput,
			grad_image,
	...
	{
		trilinear_real_projection_backward_cuda(
			image,
			rot_matrix,
			dcost_doutput,
			grad_image,
