...
#define complex_t c10::complex<scalar_t>

#define LIN_INTERP(a, l, h) ((l) + ((h) - (l)) * (a))


template <typename scalar_t>
__device__ void atomicAddComplex(complex_t* target, complex_t value)
{
	atomicAdd(( scalar_t*)target,      value.real());
	atomicAdd(((scalar_t*)target) + 1, value.imag());
}

template <typename scalar_t>
__global__ void trilinear_complex_projection_forward_cuda_kernel(
    const torch::PackedTensorAccessor32<complex_t, 3, torch::RestrictPtrTraits> image_in,
    const int size_in,
    const scalar_t* __restrict__ rot_matrix,
    const int batch_size,
    torch::PackedTensorAccessor32<complex_t, 3, torch::RestrictPtrTraits> image_out,
    const int size_out
)
{
	...
	if (xp*xp + yp*yp + zp*zp <= max_r2)
	{
		...
		// value at the opposing position.
		scalar_t conj = 1;
		...
		image_out[b][yi][xi] = complex_t(vxyz.real(), conj * vxyz.imag());
	}
}

template <typename scalar_t>
__global__ void trilinear_complex_projection_backward_cuda_kernel(
    const torch::PackedTensorAccessor32<complex_t, 3, torch::RestrictPtrTraits> image_in,
    const int size_in,
    const scalar_t* __restrict__ rot_matrix,
    const int batch_size,
    const int size_out,
    const torch::PackedTensorAccessor32<complex_t, 3, torch::RestrictPtrTraits> dcost_doutput,
	      torch::PackedTensorAccessor32<complex_t, 3, torch::RestrictPtrTraits> dcost_dimage_in,
    scalar_t* __restrict__ dcost_drot_matrix )
{
	...
	if (xp*xp + yp*yp + zp*zp <= max_r2)
	{
		...
		complex_t dC_dout = dcost_doutput[b][yi][xi];
		
		// conjugate dC_dout to account for the conjugation of the output:
		dC_dout = complex_t(dC_dout.real(), conj * dC_dout.imag());
		...
		for (int k = 0; k < 8; k++)
		{
			...
			atomicAddComplex(&dcost_dimage_in[zk][yk][xk], dC_dout * w_k);
		...
		for (int k = 0; k < 3; k++)
		{
			const scalar_t dC_dpk = 
				dC_dout.real() * dout_dp[k].real() + 
				dC_dout.imag() * dout_dp[k].imag();
...
)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        "trilinear_complex_projection_forward_cuda_kernel",
        ([&] {
            trilinear_complex_projection_forward_cuda_kernel<scalar_t><<<dimGrid, dimBlock>>>(
					image_in.packed_accessor32<complex_t, 3, torch::RestrictPtrTraits>(),
					size_in,
					rot_matrix.data_ptr<scalar_t>(),
					rot_matrix.size(0),
					image_out.packed_accessor32<complex_t, 3, torch::RestrictPtrTraits>(),
					size_out
            );
        })
    );
...
)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        rot_matrix.scalar_type(),
        "trilinear_complex_projection_backward_cuda_kernel",
        ([&] {
            trilinear_complex_projection_backward_cuda_kernel<scalar_t><<<dimGrid, dimBlock>>>(
				image_in.packed_accessor32<complex_t, 3, torch::RestrictPtrTraits>(),
				size_in,
				rot_matrix.data_ptr<scalar_t>(),
				rot_matrix.size(0),
				size_out,
                dcost_doutput.packed_accessor32<complex_t, 3, torch::RestrictPtrTraits>(),
                dcost_dimage_in.packed_accessor32<complex_t, 3, torch::RestrictPtrTraits>(),
